...
Accion Ejercicio2 es 
 ...
 Proceso 
		...
		Mientras NFDA(arch) hacer 
			...
			j:= reg.cod_suc
		 	Si i <> 0 entonces 
				promos[i,j].importe:= promos[i,j].importe + getImporte(reg.cod_prod) * reg.cant
				promos[7,j].importe:= promos[7,j].importe + getImporte(reg.cod_prod) * reg.cant
				promos[i,11].importe:= promos[i,11].importe + getImporte(reg.cod_prod) * reg.cant
				promos[7,11].importe:= promos[7,11].importe + getImporte(reg.cod_prod) * reg.cant
			FinSi
		 	Leer(arch,reg)
		FinMientras
		Para i:= 2 a 6 hacer
